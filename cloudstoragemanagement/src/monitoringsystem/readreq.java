...
public class readreq extends Thread
{
        ...
        try
        {
           ...
           while (true)
           {
               ...
               if (req.equals("LOGIN"))
               {
                   ...
                   Vector v=verifyuser(uname,pass);
                   for (int i=0;i<v.size();i++)
                   oos.writeObject(v.get(i).toString().trim());
               }
               else
               if (req.equals("FORGOTPASSWORD"))
               {
                   String uname=(String)oin.readObject();
                   
                   String reply=sendpassword(uname);
                   oos.writeObject(reply);
               }
               else 
               ...
               if (req.equals("DOWNLOAD"))
               {
                   ...
                   if (rs.next())
                   {
                       byte b[]=getfile(fname,rs.getString("storage").trim());
                       ...
                       sm.main(email,"CMS Password","Your one time password is "+ otp);
               ...
               if (req.equals("SEARCHFILE"))
               {
                   ...
                   if (rs.next())
                   {
                       ...
                       String msg=fname+" found in "+rs.getString("storage")+" having rank "+rs.getInt("rank");
                       oos.writeObject(msg);
        ...
        try
        {
           ...
           boolean exist=false;
            while (rs.next())
            {
                ...
                byte b1[]=getfile(fname,rs.getString("storage"));
    ...
    Vector verifyuser(String uname,String pass)
    {
        Vector v=new Vector();
        String reply="FAILED";
        try
        {
            ...
            if (rs.next())
            {
                ...
                String emailreply=new SendMailExample().main(email, "Login OTP", msg);
                v.add(emailreply);
                if (emailreply.equals("SUCCESS"))
                v.add(otp);
            }
            else
                v.add(reply);
    ...
    String sendpassword(String uname)
    {
        String reply="FAILED";
        try
        {
            ...
            if (rs.next())
            {
                ...
                SendMailExample sm=new SendMailExample();
                sm.main(email,"CMS Password", "Your password is "+pass);
